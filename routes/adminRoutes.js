...
router.post('/signin', [check('email').isEmail().withMessage('Please enter a valid emmail address').normalizeEmail(), check('password').isLength({ min: 8 }).isAlphanumeric().withMessage('Password must be 8 characters minimum').trim()], adminRoutes.createLogin)
